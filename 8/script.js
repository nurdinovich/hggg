...
function renderItems(array) {
    ...
    array.forEach((item)=>{
        ...
        <h3>${item.name} <h4>${item.price}</h4></h3>
        <div/>`
...
function search() {
    ...
    const searchTrem = searchInput.value.toLowerCase();
    const filteredData = data.filter((item) =>{
        return item.name.toLowerCase().includes(searchTrem)
    })
    renderItems(filteredData)
    console.log('call',  searchTrem);
}
